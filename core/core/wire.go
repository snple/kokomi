...
func (s *WireService) setValue(ctx context.Context, in *pb.WireValue, check bool) (*pb.MyBool, error) {
	...
	if err = s.setWireValue(ctx, &item, in.GetValue(), t); err != nil {
		return &output, err
	}
...
func (s *WireService) setValueByName(ctx context.Context, in *cores.WireNameValue, check bool) (*pb.MyBool, error) {
	...
	if err = s.setWireValue(ctx, &item, in.GetValue(), time.Now()); err != nil {
		return &output, err
	}
...
func (s *WireService) setWireValue(ctx context.Context, item *model.Wire, value string, updated time.Time) error {
	var err error
