...
func (s *VarService) Update(ctx context.Context, in *pb.Var) (*pb.Var, error) {
	...
	// name validation
	{
		...
		modelVar := model.Var{}
		err = s.cs.GetDB().NewSelect().Model(&modelVar).Where("device_id = ?", item.DeviceID).Where("name = ?", in.GetName()).Scan(ctx)
		if err != nil {
			if err != sql.ErrNoRows {
				return &output, status.Errorf(codes.Internal, "Query: %v", err)
			}
...
func (s *VarService) viewByDeviceIDAndName(ctx context.Context, deviceID, name string) (model.Var, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return item, status.Errorf(codes.NotFound, "Query: %v, Var DeviceID: %v, Name: %v", err, deviceID, name)
		}
